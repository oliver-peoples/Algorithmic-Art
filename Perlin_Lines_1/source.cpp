...
int main()
{
    ...
    {
        master_particles.push_back({ true,{ abs(randDouble()) * double(t_duration),abs(randDouble()) * grid_spatial_dimensions.i,abs(randDouble()) * grid_spatial_dimensions.j } });
    }
    ...
    for (size_t t_step = 0; t_step < t_duration; t_step += t_step_size)
    {
        ...
        cv::Mat frame(img_size, CV_8UC4, { 0,0,0,0 });
        ...
        while (remaining_particles > 0 && iteration < iteration_limit)
        {
            for (size_t particle_n = 0; particle_n < t_step_particles.size(); particle_n++)
            {
                if (t_step_particles[particle_n].simulated == true)
                {
                    ...
                    {
                        cv::line(frame, { old_position.j * grid_img_map.i,old_position.k * grid_img_map.j }, { t_step_particles[particle_n].position.j * grid_img_map.i,t_step_particles[particle_n].position.k * grid_img_map.j }, { 255,0,0,255 }, 2);
                    }
